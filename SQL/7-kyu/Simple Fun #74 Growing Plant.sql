...
SELECT
  id,
  CASE WHEN desired_height <= up_speed
       THEN 1
       WHEN (desired_height - down_speed) % (up_speed - down_speed) = 0
       THEN ((desired_height - down_speed) / (up_speed - down_speed))::INTEGER
       ELSE (((desired_height - down_speed) / (up_speed - down_speed)) + 1)::INTEGER END AS num_days
FROM growing_plant;