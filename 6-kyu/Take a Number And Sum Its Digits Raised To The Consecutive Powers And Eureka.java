...
class SumDigPower { 
  public static List<Long> sumDigPow(long a, long b) {
    ...
    for (long number = a; number <= b; number++) {
      ...
      for (int power = 1; power <= numberStr.length(); power++) {
        int digit = Integer.parseInt(String.valueOf(numberStr.charAt(power - 1)));
        long digitPowered = (long) Math.pow(digit, power);
        sum += digitPowered;
      }
