...
public class MySolution {
  public int computeDepth(int n) {
    return calculateDepth(n, new TreeSet(), 1);
  }
  
  public static int calculateDepth(int n, Set<String> digits, int multiplier) {
    Arrays.stream(String.valueOf(n * multiplier).split(""))
      .forEach(digit -> digits.add(digit));
    
    return digits.size() == 10 ? multiplier 
      : calculateDepth(n, digits, ++multiplier);
  }
}