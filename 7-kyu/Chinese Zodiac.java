...
public class Kata extends Arrays {
  public static String chineseZodiac(int year) {
    final int START_YEAR = 1924;
    final int ELEMENTS_CHANGE = 2;
    
    int yearFromStart = year - START_YEAR;
    
    int elemIndex = (yearFromStart / ELEMENTS_CHANGE) % elements.length;
    int animalIndex = yearFromStart % animals.length;
    
    return elements[elemIndex] + " " + animals[animalIndex];
  }
}