...
public class Solution {
  public static String solveRockOff(final int[] alice, final int[] bob) {
    int aliceWins = getWinsOfFirst(alice, bob);
    int bobWins = getWinsOfFirst(bob, alice);
    
    return aliceWins + ", " + bobWins + ": " +
      (aliceWins > bobWins ? "Alice made \"Kurt\" proud!"
      : aliceWins < bobWins ? "Bob made \"Jeff\" proud!"
      : "that looks like a \"draw\"! Rock on!");
  }
  
  private static int getWinsOfFirst(int[] first, int[] second) {
